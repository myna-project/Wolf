...
class ConfigParserMultiOpt(configparser.RawConfigParser):
  ...
  def __init__(self):
    super(ConfigParserMultiOpt, self).__init__(self, empty_lines_in_values=False, strict=False, dict_type=dict)
    ...
    for key, value in section_items:
        ...
        if isinstance(value, tuple):
            for multivalue in value:
                multivalue = delimiter + str(multivalue).replace(u'\n', u'\n\t')
                fp.write(u"{0}{1}\n".format(key, multivalue))
            continue
