...
class dlms_net():
    ...
    params = [{'name': 'deviceid', 'type': 'string', 'required': True},
            ...
            {'name': 'port', 'type': 'int', 'default': 4060, 'required': True},
    ...
    def __init__(self, name):
        ...
        self.media = GXNet(NetworkType.TCP, self.host, self.port)
    ...
    def poll(self):
        if not self.cached:
            ...
            return []
        list_ = list()
        objs = self.client.objects.getObjects([ObjectType.REGISTER, ObjectType.EXTENDED_REGISTER, ObjectType.DEMAND_REGISTER, ObjectType.CLOCK])
        for obj in objs:
            list_.append((obj, 2))
        ut = time.time()
        self.open()
